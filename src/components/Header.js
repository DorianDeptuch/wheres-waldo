import React from "react";
import Timer from "./Timer";
import magikarp from "../images/magikarp-pixel.png";
import waldo from "../images/waldo-pixel.png";
import bluePortal from "../images/blue-portal-pixel.png";
import angryBird from "../images/angry-bird-pixel.png";

function Header({
  showGame,
  setShowRemaining,
  showRemaining,
  timer,
  setTimer,
  foundMagikarp,
  foundWaldo,
  foundBluePortal,
  foundAngryBird,
}) {
  return (
    <header className="header">
      <div className="waldo-title">
        <h1>
          <span>WHERE'S</span> <span>WALDO?</span>
        </h1>
      </div>
      {showRemaining ? (
        <div className="game-stats-container">
          <div className="characters-remaining-container">
            <div className="remaining">Remaining:</div>
            <div style={foundMagikarp ? { opacity: 0.25 } : null}>
              <img src={magikarp} alt="test"></img>
            </div>
            <div style={foundWaldo ? { opacity: 0.25 } : null}>
              <img src={waldo} alt="test"></img>
            </div>
            <div style={foundBluePortal ? { opacity: 0.25 } : null}>
              <img src={bluePortal} alt="test"></img>
            </div>
            <div style={foundAngryBird ? { opacity: 0.25 } : null}>
              <img src={angryBird} alt="test"></img>
            </div>
          </div>
          {/* <div className="timer-container">{timer}</div> */}
          <Timer timer={timer} setTimer={setTimer} />
        </div>
      ) : null}
      <div className="github-link">
        <a
          href="https://github.com/DorianDeptuch"
          target="_blank"
          rel="noreferrer"
        >
          <img
            src="data:image/png;base64,"
            alt="github icon"
          ></img>
        </a>
      </div>
    </header>
  );
}

export default Header;
